...
def localization_to_rat_dyadic : localization_map.away_map 2 rat.dyadic :=
{ to_fun         := λ n : ℤ, (n : rat.dyadic),
  ...
  map_units'     := begin
                      ...
                      use [b, a],
                      simp only [rat.dyadic.coe_eq_iff, subring.coe_mul, subring.coe_one,
                                subring.coe_int_cast, subtype.coe_mk, subtype.val_eq_coe],
                      have : b = ((2 ^ a) : ℚ)⁻¹,
                        by { rw [eq_inv_iff, rat.inv_def'],
  ...
  surj'          := begin
                      rintro ⟨a, b, h⟩,
                      use [a.num, 2 ^ b, b],
                      simp [rat.dyadic.coe_eq_iff],
                      norm_cast,
                      rw [← h, rat.mul_denom_eq_num],
                    end,
  eq_iff_exists' := begin
                      intros x y,
                      norm_cast,
...
#check localization_to_rat_dyadic.to_map
#check localization_map.injective
